...
namespace TugasBesar
{
    public class Pegawai
    {
        ...
        public static Pegawai Select(int id_pegawai)
        {
            ...
            conn.Open();
            cmd = conn.CreateCommand();
            cmd.CommandText = "SELECT a.username, a.password, a.role, p.nama_pegawai FROM akun a INNER JOIN data_pegawai p " +
            ...
            try
            {
                ...
                {
                    rdr.Read();
                    pegawai.username = rdr.GetString(0);
                    pegawai.password = rdr.GetString(1);
                    pegawai.role = rdr.GetInt32(2);
                    pegawai.nama_pegawai = rdr.GetString(3);
                }
            }
            catch (Exception e)
            {
